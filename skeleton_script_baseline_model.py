...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Generates baseline model predictions."
    )
    # Arguments
    ...
    parser.add_argument(
        "-o",
        dest="out_path",
        help="a path to write the output .tsv file with baseline model scores. "
...
def parse_blosum(path):
    ...
    blosum_dict = {
        aas[i]: {aas[j]: aa_scores[i][j] for j in range(len(aas))}
        for i in range(len(aas))
    }

    blosum_dict_for = {}
    for i in range(len(aas)):
        blosum_dict_for[aas[i]] = {}
        for j in range(len(aas)):
            blosum_dict_for[aas[i]][aas[j]] = aa_scores[i][j]

    assert blosum_dict == blosum_dict_for
    return blosum_dict
...
def main():
    ...
    if ".tsv" not in out_filename:
        sys.exit(
            r'ERROR: filename "%s" in the output file path argument should contain .tsv extension!'
            % out_filename
        )
    ...
    if not os.path.exists(out_dir):
        sys.exit(
            r'ERROR: output directory "%s" to store baseline model output does not exist! Follow instructions in'
